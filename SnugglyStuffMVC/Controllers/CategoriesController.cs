using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.IO;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using SnugglyStuffMVC.Models;

namespace SnugglyStuffMVC.Controllers
{
    [Authorize]
    public class CategoriesController : Controller
    {
        private SnugglyStuffEntities db = new SnugglyStuffEntities();

        // GET: Categories
        public ActionResult Index()
        {
            return View(db.Categories.ToList());
        }

        // GET: Categories/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Category category = db.Categories.Find(id);
            if (category == null)
            {
                return HttpNotFound();
            }
            return View(category);
        }

        // GET: Categories/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: Categories/Create
        // To protect from overposting attacks, enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(Category category, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                var Data = GetCategoryDataList();

                int MaxId = Data[0].ID + 1;

                string ImageName = "Category" + MaxId + ".jpg";

                string path = Path.Combine(Server.MapPath("~/Images/Categories/"), ImageName);

                if (file != null)
                {
                    file.SaveAs(path);
                }

                category.Image = ImageName;

                db.Categories.Add(category);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(category);
        }

        public List<Category> GetCategoryDataList()
        {
            var CategoriesList = db.Categories.SqlQuery("SELECT " +
                                          "convert(int,IDENT_CURRENT ('Categories')) as ID, " +
                                          "convert(varchar(MAX),IDENT_CURRENT('Categories')) as Image," +
                                          "convert(varchar(MAX),IDENT_CURRENT('Categories')) as CategoryName")
                                          .ToList<Category>();

            return CategoriesList;
        }

        // GET: Categories/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Category category = db.Categories.Find(id);
            if (category == null)
            {
                return HttpNotFound();
            }
            return View(category);
        }

        // POST: Categories/Edit/5
        // To protect from overposting attacks, enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Category category, HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                var ImageName = category.Image;

                string path = Path.Combine(Server.MapPath("~/Images/Categories/"), ImageName);

                if (file != null)
                {
                    file.SaveAs(path);
                }

                db.Entry(category).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(category);
        }

        // GET: Categories/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            Category category = db.Categories.Find(id);
            if (category == null)
            {
                return HttpNotFound();
            }
            return View(category);
        }

        // POST: Categories/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            Category category = db.Categories.Find(id);
            db.Categories.Remove(category);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
